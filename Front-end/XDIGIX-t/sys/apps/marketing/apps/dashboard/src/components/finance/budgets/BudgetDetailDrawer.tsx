  ...
  <Transition.Root show={open} as={Fragment}>
    <Dialog as="div" className="relative z-40" onClose={onClose}>
      ...
      <div className="fixed inset-0 right-0 flex max-w-xl">
        ...
        >
          <Dialog.Panel className="flex h-full w-full flex-col border-l border-gray-100 bg-white shadow-xl">
                ...
                <section className="rounded-2xl border border-primary/10 bg-primary/5 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-xs font-semibold uppercase text-madas-text/50">Spent</p>
                    ...
                    {transactions.length ? (
                      transactions.map((transaction) => (
                        <li key={transaction.id} className="flex items-center justify-between text-sm">
                          <div>
                            <p className="font-medium text-primary">{transaction.description ?? 'Expense'}</p>
                            <p className="text-xs text-madas-text/60">{transaction.createdAt.toLocaleDateString()}</p>
                          </div>
