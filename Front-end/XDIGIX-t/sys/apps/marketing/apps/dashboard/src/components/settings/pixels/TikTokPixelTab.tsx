...
interface TikTokPixelTabProps {
  pixels: PixelItem[];
  onSave: (pixels: PixelItem[]) => Promise<boolean>;
  saving: boolean;
}

const TikTokPixelTab = ({ pixels, onSave, saving }: TikTokPixelTabProps) => {
  ...
  const handleSave = async () => {
    ...
    if (success) {
      alert('TikTok Pixels saved successfully!');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-primary mb-2">TikTok Pixel</h2>
        <p className="text-sm text-madas-text/70">
          Add and manage your TikTok Pixel IDs for tracking.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {localPixels.length === 0 ? (
          <div className="text-center py-8 text-madas-text/60">
            <span className="material-icons text-4xl mb-2 block">video_library</span>
            <p>No TikTok Pixels configured yet.</p>
            <p className="text-sm mt-1">Click "Add TikTok Pixel" to get started.</p>
          </div>
        ) : (
            ...
            <div key={pixel.id} className="border border-gray-200 rounded-lg p-4 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-medium text-madas-text/80">
                  {pixel.name || `TikTok Pixel ${localPixels.indexOf(pixel) + 1}`}
                </h3>
                <button
                  type="button"
