...
const SnapchatPixelTab = ({ pixels, onSave, saving }: SnapchatPixelTabProps) => {
  ...
  const handleSave = async () => {
    ...
    if (success) {
      alert('Snapchat Pixels saved successfully!');
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-xl font-semibold text-gray-900 mb-2">Snapchat Pixel</h2>
        <p className="text-sm text-gray-500">
          Add and manage your Snapchat Pixel IDs for tracking.
        </p>
      </div>
      ...
      <div className="space-y-4">
        {localPixels.length === 0 ? (
          <div className="text-center py-8 text-gray-400">
            <span className="material-icons text-4xl mb-2 block">camera_alt</span>
            <p>No Snapchat Pixels configured yet.</p>
            <p className="text-sm mt-1">Click "Add Snapchat Pixel" to get started.</p>
          </div>
        ) : (
            ...
            <div key={pixel.id} className="border border-gray-200 rounded-lg p-4 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-medium text-gray-700">
                  {pixel.name || `Snapchat Pixel ${localPixels.indexOf(pixel) + 1}`}
                </h3>
                <button
                  type="button"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Main Campaign Pixel"
                  />
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Snapchat Pixel ID <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={pixel.pixelId}
...
export default SnapchatPixelTab;
