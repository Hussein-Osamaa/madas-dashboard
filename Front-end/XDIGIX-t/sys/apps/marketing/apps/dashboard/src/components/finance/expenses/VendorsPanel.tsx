...
const VendorsPanel = ({ vendors, totalSpent, currency }: Props) => (
  <div className="space-y-4">
    ...
    <ul className="space-y-3">
      {vendors.length ? (
        vendors.map((vendor) => (
          <li key={vendor.id} className="rounded-2xl border border-gray-100 bg-white px-4 py-3 shadow-sm">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-madas-text/60">
                  Last expense: {vendor.lastExpense ? vendor.lastExpense.toLocaleDateString() : '—'}
                </p>
              </div>
            </div>
          </li>
        ...
        <li className="rounded-2xl border border-dashed border-gray-200 bg-white px-4 py-3 text-xs text-madas-text/60">
