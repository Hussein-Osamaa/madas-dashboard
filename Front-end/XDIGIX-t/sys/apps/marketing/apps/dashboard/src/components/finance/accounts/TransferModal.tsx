...
import { createTransfer } from '../../../services/finance/accountsService';
...
const TransferModal = ({ open, onClose, accounts }: Props) => {
  ...
  const mutation = useMutation({
    mutationFn: (payload: FormValues) =>
      createTransfer(scope.businessId!, {
        ...payload,
                ...
                <form className="mt-5 space-y-4" onSubmit={handleSubmit(onSubmit)}>
                  <div>
                    ...
                    {errors.fromAccountId ? (
                      <p className="mt-1 text-xs text-rose-600">{errors.fromAccountId.message}</p>
                    ) : null}
                  </div>
                  <div>
                    ...
                    {errors.toAccountId ? <p className="mt-1 text-xs text-rose-600">{errors.toAccountId.message}</p> : null}
                  </div>
                  ...
                  <div className="rounded-2xl bg-primary/10 px-4 py-3 text-sm font-semibold text-primary">
                    You’re moving {formatCurrency(amount || 0, currency)} between internal accounts.
                  </div>
